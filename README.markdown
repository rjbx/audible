...
For greater control, intermediate processing can be managed from a single [`BiquadFilterNode`](https://developer.mozilla.org/en-US/docs/Web/API/BiquadFilterNode) that encompasses frequency, gain, and several other attributes, in which case, destination nodes should connect to it rather than Gain for the filtered output.
